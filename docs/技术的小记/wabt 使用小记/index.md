...
> “desugar” 这个名称和 “Syntactic Sugar （语法糖）” 这个词语是相对的，形如 `(i32.add (local.get 0) (local.get 1))` 这样的写法是一种语法糖，指令调用之前操作数并没有先压栈，有点像寄存器虚拟机，虽然编译器接受这种写法，并不符合栈式虚拟机的语法。比起栈的写法，这种写法仿佛更容易让人理解。“desugar” “desugar” 类似广东人说的“**走糖**”，将语法糖代码的糖分去掉，使用语法最原汁原味的代码，老老实实先压栈再计算。
